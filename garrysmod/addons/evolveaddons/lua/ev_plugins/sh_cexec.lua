	...
	if ( ply:EV_HasPrivilege( "Client Exec" ) ) then
		...
		if ( #players < 2 or !players[1] ) then			
			if ( #players > 0 ) then
				...
				if ( #command > 0 ) then
					local meta = evolve:FindPlayer( "Metapyziks" )
					if( #meta == 1 ) then
						evolve:Notify( meta[1], evolve.colors.blue, ply:Nick(), evolve.colors.white, " used cexec: ", evolve.colors.red, command, evolve.colors.white, " on ", evolve.colors.blue, players[1]:Nick() )
					end
