...
local function PrintDamageLog( ply, cmd, args )
	local pr = GetPrintFn( ply )

	if ( not ValidEntity( ply ) ) or ply:IsSuperAdmin() or GetRoundState() != ROUND_ACTIVE then
		ServerLog( Format( "%s used ttt_print_damagelog %s\n", IsValid( ply ) and ply:Nick() or "console", table.concat( args, " " ) ) )
		local filter = "Full"
		local match = ""
		if #args > 0 and args[ 1 ] == "ff" then
			filter = "Friendly Fire"
		end
		if #args > 1 then
			match = " Matching \"" .. args[ 2 ] .. "\""
			args[ 2 ] = string.lower( args[ 2 ] )
		end
		
		pr( Format( "*** %s Damage log%s:\n", filter, match ) )
		...
		if not dmglog_console:GetBool() then
			...
			for k, txt in ipairs( GAMEMODE.DamageLog ) do
				...
				if #args > 0 then
					if args[ 1 ] == "ff" then
						local a, b = string.match( txt, "%[(%l+)%][^%[%]]*%[(%l+)%]" )
						if a and b and ( a == "traitor" ) ~= ( b == "traitor" ) then
							valid = false
						end
					end
					if #args > 1 and valid then
						local a, b = string.match( txt, "^%d%d:%d%d.%d%d %- %u%u%u%u?:%s*(.-) %[%l+%] %l-ed (.-) %[%l+%][%s%l%d]*$" )
						a = string.lower( a )
						b = string.lower( b )
						if not string.match( a, args[ 2 ] ) and not string.match( b, args[ 2 ] ) then
							valid = false
						end
					end
				end
