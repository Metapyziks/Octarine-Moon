...
function PANEL:UpdateTag()
	if GAMEMODE.round_state == ROUND_ACTIVE and LocalPlayer():Alive()
		and LocalPlayer():GetDetective() then
		local tagtxt = "sb_tag_none"
		if self.Player.sb_tag then
			tagtxt = self.Player.sb_tag.txt
		end
		RunConsoleCommand( "ttt_detective_tag", self.Player:EntIndex(), tagtxt )
	end
