...
--if not ConVarExists( "ttt_knife_damage" ) then
	SWEP.Damage = CreateConVar( "ttt_knife_damage", "100", { FCVAR_REPLICATED, FCVAR_NOTIFY } )
--else
--	SWEP.Damage = GetConVar( "ttt_knife_damage" )
--end
...
function SWEP:PrimaryAttack()
   ...
   if SERVER and tr.Hit and tr.HitNonWorld and ValidEntity(hitEnt) then
      if hitEnt:IsPlayer() then
		 ...
		 local damage = self.Damage:GetInt()
         if hitEnt:Health() < (damage + 10) then
...
if CLIENT then
   function SWEP:DrawHUD()
      ...
      if tr.HitNonWorld and IsValid(tr.Entity) and tr.Entity:IsPlayer()
         and tr.Entity:Health() < (self.Damage:GetInt() + 10) then
