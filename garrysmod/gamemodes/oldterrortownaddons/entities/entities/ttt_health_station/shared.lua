...
function ENT:Poison( gun )
	if( not self.Poisoned ) then
		self.Poisoned = true
		self.PoisonTime = CurTime() + 120
	else
		self.PoisonTime = self.PoisonTime + 240
	end
	self.Poisoner = gun
	self.Entity:SetColor( 128, 255, 160, 255 )
	
	--HIGHSCORE:AddScore( gun.Owner, HIGHSCORE.PointsPerPoison )
end
...
function ENT:GiveHealth(ply, max_heal)
	if( self.Poisoned and CurTime() > self.PoisonTime ) then
		self.Poisoned = false
		self.Entity:SetColor(180, 180, 255, 255)
	end

	if( self.Poisoned ) then
		self.Poisoner:PoisonPlayer( ply, 120 )
	end
